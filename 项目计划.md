...
| **P3** | ✅ Gate（Conflict 重试、metrics/JSON 日志、退避、uncordon、CI e2e-kind continue-on-error）；**L1-B** 本地 `e2e-kind.sh` / `e2e-promql.sh` 需 Docker+kind |
| P4 代码 | ⬜ 演示手册 / CHANGELOG |
| ADR-0001 | ✅ |
| CHANGELOG | ⬜ P4 |
| CI on `dev` | ✅ `test` + `e2e-kind`（`continue-on-error: true`） |

**下一步：** P4 演示与 CHANGELOG。
