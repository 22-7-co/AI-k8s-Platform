...
| P3 起 | 增加 `e2e-kind`；P4 起 **required**（`continue-on-error: false`） |
...
| **P2** | ✅ **L1-A Plan B**（e2e-k3s 绿） |
| **P3** | ✅ L1-B + 硬化（e2e-kind / e2e-promql 本地 2026-05-25） |
| **P4** | ✅ demo.sh、interview-pitch、CHANGELOG、CI e2e-kind **required** |
| **P5-Obs** | ✅ Grafana + Prometheus scrape Operator（本地 Docker 演示栈） |
| ADR-0001 | ✅ |
| CHANGELOG | ✅ 首版 |
| CI on `dev` | ✅ `test` + `e2e-kind`（required） |

**下一步：** `dev` → `main` PR，合并由维护者决定。
...
- [x] P0 Gate：healing 单测 ≥60% 绿  
- [x] P1 Gate：prometheus 单测 ≥60% 绿  
- [x] P2 Gate（L1-A）：e2e-k3s 绿（Plan B，2026-05-21）  
- [x] P3 Gate（L1-B）：e2e-kind 2 节点 + 集群内 Operator + e2e-promql 真 PromQL 子路径  
- [x] P2-6 / P3：Events 审计 + uncordon + 退避 + CI dev  
- [x] P4：demo.sh + interview-pitch + CHANGELOG + CI e2e-kind required（2026-05-25）  
- [ ] P4 合并：dev → `main` PR（维护者手动）  
