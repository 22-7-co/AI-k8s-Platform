...
> **变更说明：** v1.1 拍板 MVP 控制面输入、模块边界、成功标准分级、社区对标、安全/测试 gate、文档治理
...
（同 v1.0：长周期训练、单卡 XID → NCCL 失败、人工重启不可扩展。）
...
| Operator 挂了 | `operator_up`、Deployment 事件 | 重启；**MVP 不自愈 Operator 自身** |
...
| `pods` | get, list, watch, delete | 驱逐 |
| `pods/eviction` | create | 可选优雅驱逐 |
...
| 阶段 | 名称 | 周期（参考） |
|------|------|--------------|
| **P0** | K8s API + healing 单测 | 1–2 周 |
| **P1** | mock 指标 + PromQL | 1 周 |
| **P2** | 闭环 MVP（L1 验收） | 2 周 |
| **P3** | 硬化 + e2e + 回滚脚本 | 1 周 |
| **P4** | 演示 L2 + 面试稿 + 合并 main | 1 周 |
...
| P0-2 | `Taint` / `Label` | 同上 |
| P0-3 | 幂等骨架（内存态或 label） | 重复调用不重复 patch |
...
| P2-2 | healing 串联 Cordon→Taint→驱逐 | 坏节点训练 Pod 消失 |
...
| 产品初衷 | `说明文档.txt` | ✅ |
| 本计划 | `项目计划.md` | ✅ v1.1 |
| ADR | `docs/adr/` | ⬜ P2 首条：MVP 仅 PromQL Pull |
...
> 我实现了一个 **GPU 故障闭环 Operator**：MVP 用 **PromQL 轮询 + mock XID** 发现故障节点，**client-go** 完成 Cordon、污点与训练 Pod 驱逐，并用 Pod Informer **确认 Job 在新节点 Running**。平台交付的是**调度侧自愈**；断点续训在示例 Job 的 PVC 契约里演示，不夸大 Operator 能力。与 NPD、Cluster Autoscaler、官方 DCGM Exporter 分工不同：我补的是 **指标 → 编排 → 工作负载迁移** 这一段。
...
- 架构拍板（如 §3.0）写入 `docs/adr/0001-mvp-promql-pull-only.md`（P2-1 前完成）。  
...
| 机制 | 说明 |
|------|------|
| §15 快照表 | 每完成 Px Gate，更新状态与日期 |
| 周更 | 每周或每阶段结束更新 §15 + 勾选附录 B |
...
| P0–P4 代码 | ⬜ |
| ADR / CHANGELOG | ⬜ |
| CI on `dev` | ⬜ 待改 workflow |

**下一步：** P0-1 `Cordon` + P0-5 Gate 单测。
...
- [ ] P1 Gate：prometheus 单测 ≥60% 绿  
- [ ] P2 Gate：e2e-kind 绿 + L1（Job Running）  
- [ ] P2-6：Events 审计  
...
*v1.1：回应控制面输入、controller 边界、续训分级、社区对标、安全审计、零 CRD、测试 Gate、可观测排障、文档治理。*
